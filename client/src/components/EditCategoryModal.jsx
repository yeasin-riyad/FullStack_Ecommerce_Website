...
import { editCategory } from "../store/productSlice";

const EditCategoryModal = ({category,close}) => {
    ...
    const [data, setData] = useState({
      ...
      const handleSubmit = async (e) => {
        ...
        formData.append("categoryId", _id); // Append the id of the category to the form data
        formData.append("name", data.name);
        formData.append("image", data.image); // Append the file
      
        try {
          const res = await axiosSecure.put("category/update-category", formData, {
            // headers: {
     ...
     <section className="fixed p-4 top-20 lg:top-9 left-0 right-0 bottom-0 bg-neutral-800 bg-opacity-60 flex items-center justify-center">
       <div className="bg-white w-full p-4 rounded max-w-4xl">
         ...
         <form className="my-3" onSubmit={handleSubmit}>
           ...
           <button
             type="submit"
             className="w-full mt-4 bg-primary-200 text-white px-4 py-2 rounded"
             disabled={ loading }// Disable if name or image is missing
           >
             {loading ? "Waiting......." : "Update Category"}
           </button>
         </form>
       </div>
     </section>
