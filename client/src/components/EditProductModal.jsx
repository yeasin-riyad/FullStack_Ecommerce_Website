...
const EditProductModal = ({data:product,onClose,fetchProducts}) => {
      ...
      try{
        setProductUploading(true);
          ...
          if(!data?.name || data?.image.length===0 || data?.category.length===0 
            || data?.subCategory.length===0 || !data?.unit || !data?.price || !data?.description) {
          ...
          const result = await axiosSecure.put('product/edit-product', formData)
          toast.success(result?.data?.message)
          onClose()
          // Fetch the updated product data from the server to update the Redux state.
          fetchProducts()
          // setData({
          //   name: "",
          //   image: [],
  ...
  return (
    <section className="fixed p-4 top-20 left-0 right-0 bottom-0  bg-neutral-800 bg-opacity-60 flex items-start justify-center overflow-y-scroll h-full max-h-[90vh] ">
      ...
      <div className="p-2 bg-white shadow-md flex items-center justify-between">
        ...
        <button className="text-primary-200 block ml-auto" onClick={onClose}>
